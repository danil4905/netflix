...
let state = {
    progress: 0, downloadSpeed: 0, ratio: 0
}
...
router.get('/add/:magnet', (req: Request, res: Response) => {
    ...
    client.add(magnet, (torrent) => {
        const files = torrent.files.map(el => ({name: el.name, length: el.length}))
...
router.get('/:magnet/:fileName', (req: StreamRequest, res: Response, next: NextFunction) => {
    const {
        params: {magnet, fileName}, headers: {
            range
        }
    } = req
    if (!range) {
        const error = <ErrorWithStatus>new Error('Range is not defined, please make request from HTML5 Player')
        error.status = 416
        return next(error)
    }
    // file searching
